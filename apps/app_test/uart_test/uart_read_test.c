...
void receivethread(void)
{
  ...
  {
	memset(buff, 0, 512); 
	if((nread = read(fd,buff,3))>0) //接收数据
	{
	    printf("[RECEIVE] First is 0x%x\n",buff[0]);
            if ((buff[0] == 0x5A) && (buff[1] == 0xA5) && (buff[2] > 0)) {
		data_len = buff[2];
		int i = 0;
		if((nread = read(fd,buff,data_len))>0) {
                        printf("Read data:");
                        while(i < data_len){
                                printf("0x%2x", buff[i]);
                                i++;
                        ...
                        printf("\n");
	        system("fpga_read_write /dev/fpga_printer_ctl 1 0 0x55 0xaa");
		}
	    if (i == 1)
	        system("fpga_read_write /dev/fpga_printer_ctl 1 0 0xaa 0x55");
 	    }	
	}
